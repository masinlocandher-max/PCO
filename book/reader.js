(function(){
  ...
  function initInstall(){
    ...
    function baseCopy(){
      ...
      copy.innerHTML=isIOS
        ?'<strong>Add to Home Screen</strong>On iPhone or iPad, tap Safari’s Share button, then choose <b>Add to Home Screen</b>.'
        :'<strong>Keep your ebook on your Home Screen</strong>Open your copy in one tap, like an app. Protected chapters still require your verified access.';
    }
    ...
    showInstallForReader=function(){
      ...
      invite.setAttribute('data-platform',isIOS?'ios':'browser');
      if(installBtn){installBtn.hidden=isIOS;installBtn.textContent='Add';}
    ...
    addEventListener('beforeinstallprompt',function(e){
      ...
      if(entitled)setTimeout(showInstallForReader,350);
    });
  ...
  async function api(action,input){
    ...
    var r=await fetch(BOOK_API,{method:'POST',headers:{apikey:SUPABASE_KEY,Authorization:'Bearer '+s.access_token,'content-type':'application/json',Accept:'application/json'},body:JSON.stringify(Object.assign({action:action},input||{})),cache:'no-store'}),data={};try{data=await r.json();}catch(e){}
  ...
  async function loadRemoteProgress(session){
    ...
    try{var url=SUPABASE_URL+'/rest/v1/book_reader_progress?select=chapter_sequence,scroll_percent,updated_at&user_id=eq.'+encodeURIComponent(claims.sub)+'&book_slug=eq.'+BOOK_SLUG+'&limit=1',r=await fetch(url,{headers:{apikey:SUPABASE_KEY,Authorization:'Bearer '+session.access_token,Accept:'application/json'},cache:'no-store'});if(!r.ok)return null;var rows=await r.json();return rows&&rows[0]?rows[0]:null;}catch(e){return null;}
  }
  async function saveRemoteProgress(keepalive){
    if(!entitled||!activeSequence||!activeUserId)return;
    var s=currentSession();if(!s||!s.access_token)return;var percent=Number(scrollPercent().toFixed(2)),url=SUPABASE_URL+'/rest/v1/book_reader_progress?on_conflict=user_id,book_slug';
  ...
  async function activateReader(){
    ...
    try{var access=await api('access');entitled=access.has_access===true&&access.content_ready===true;manifest=Array.isArray(access.chapters)?access.chapters:[];if(btn)btn.textContent=entitled?'My ebook':'Account';refreshAccessModal();if(!entitled){showToast(access.content_ready===false?'Your ebook is being prepared.':'Signed in. This copy has not been activated yet.');return;}buildToc(manifest);var remote=await loadRemoteProgress(session),wanted=remote?Number(remote.chapter_sequence):Number(prefs.sequence)||1,percent=remote?Number(remote.scroll_percent)||0:Number(prefs.percent)||0,row=manifest.find(function(x){return Number(x.sequence)===wanted&&x.kind!=='part';})||manifest.find(function(x){return x.kind!=='part';});if(row)await openChapter(row.sequence,percent);setTimeout(showInstallForReader,500);}catch(e){if(e.message==='sign_in_required'){removeKey(AUTH_STORE);activeUserId=null;if(btn)btn.textContent='Reader access';}else showToast('Reader access could not be checked right now.');}
  }
